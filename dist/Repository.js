...
import { createError, log, sha1, strComp } from "./util.js";
...
export default class Repository {
    ...
    headHash;
    headCommit;
    constructor(head, headHash, headCommit) {
        this.head = head;
        this.headHash = headHash;
        this.headCommit = headCommit;
    }
    static createFile = async (filename) => writeFile(join(this.WORKING_PATH, `${filename}.txt`), filename);
    static modifyFile = async (filename) => appendFile(join(this.WORKING_PATH, `${filename}.txt`), filename);
    static removeFile = async (filename) => rm(join(this.WORKING_PATH, `${filename}.txt`), { recursive: true });
    static readFile = async (filename) => readFile(join(this.WORKING_PATH, `${filename}.txt`), { encoding: 'utf-8' });
    static async open() {
    ...
    static async init() {
        ...
        await mkdir(this.GITLET_PATH, { recursive: true });
    ...
    async log(hash) {
        ...
        log(`===
commit ${hash}${commit.parent1 && commit.parent2
...
`);
    ...
    static getWorkingFiles = async () => readdir(this.WORKING_PATH, {
        withFileTypes: true
    })
        .then(filenames => filenames.filter(filename => filename.isFile())
        .map(filename => readFile(join(this.WORKING_PATH, filename.name))
    ...
    async status() {
        ...
        const { staged, removed } = await readdir(Repository.STAGING_PATH)
            .then(filenames => filenames.reduce((obj, filename) => {
            if (filename.includes('[REMOVE]')) {
        ...
        const modified = Object.keys(this.headCommit.files).reduce((modified, filename) => {
...
`);
    ...
    branchExists = (branchName) => existsSync(join(Repository.BRANCHES_PATH, branchName));
    ...
    async merge(branchName) {
        ...
        ].reduce((arr, files) => arr.concat(Object.keys(files)), []));
        ...
        for (const filename of allFilenames) {
            ...
            if (splitHash && currentHash && givenFileHash && splitHash !== givenFileHash && splitHash === currentHash) {
                ...
                mergeConflict = true;
                await writeFile(join(Repository.WORKING_PATH, filename), `<<<<<<< HEAD
